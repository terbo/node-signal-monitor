...
var wifiDevice = function(packet = null, device = null, max_rssis = 32) {
  ...
  if(packet) {
    ...
    try { this.sessions = new deviceSession(packet) }
    catch (e) { console.error(`Getting session for ${packet.mac}: ${e}`) }
    ...
    this.tags = ['new']
    this.lastprobe = device.lastprobe
    ...
    this.sensors = device.sensors
    try { this.sessions = new deviceSession(false, device.sessions.sessions) }
    catch (e) { console.error(`Getting session for ${packet.mac}: ${e}`) }
  } else {
    console.error('no packet sent!')
...
//      this.hosts.push(packet.ssid)
    ...
    if(packet.rftype[0] === 0)
      if(packet.rftype[1] === 0)
        output.action = `associating to ${packet.dst}`,
        output.dir    = '>'
      else if(packet.rftype[1] == 1)
        output.dir = '<',
        output.action = 'assoc response'
      else if(packet.rftype[1] == 2)
        output.action = `re-associating to ${packet.dst}`,
        output.dir    = '>'
      else if(packet.rftype[1] == 4)
        output.action = `probed ${this.ssid}`,
        output.dir    = '>',
        this.lastprobe = { ssid: packet.ssid, bssid: packet.src, time: now },
        output.client = packet.src
      else if(packet.rftype[1] == 10)
        output.action = `disassociating from ${packet.dst}`,
        output.dir    = '<'
      else if(packet.rftype[1] == 12)
        output.action = `de-authenticating from ${packet.dst}`,
        output.dir    = '-'
      else
        output.action = `${packet.rftype[0]}:${packet.rftype[1]}`,
        output.dir = '?'
  } else {                                                  // access point
    ...
    if(packet.rftype[0] === 0)
      if(packet.rftype[1] === 0)
        output.dir = '>',
        output.action = 'assoc request'
      else if(packet.rftype[1] == 3)
        output.dir = '<',
        output.action = 'reassoc response'
      else if(packet.rftype[1] == 5)
        output.dir = '=',
        output.action = 'probe response',
        output.client = packet.src,
        output.cloaked = this.ssid == '[hidden]' ? true : false
      else if(packet.rftype[1] == 8)
        output.dir = '.',
        output.action = 'beaconed'
      else if(packet.rftype[1] == 11)
        output.dir = '+',
        output.action = 'authed'
  }
  ...
  } catch(e) { console.error(`Updating ${this.mac}: ${e}`); output.error = e }
